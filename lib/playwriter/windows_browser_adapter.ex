defmodule Playwriter.WindowsBrowserAdapter do
  ...
  defp get_possible_endpoints(port) do
    base_endpoints = [
      "ws://localhost:#{port}/",
      "ws://127.0.0.1:#{port}/",
      # Common WSL interface IP
      "ws://172.19.176.1:#{port}/",
  ...
  defp find_available_port(ports) do
    # Fallback to last port if none found
        ...
        endpoint ->
          ...
          case test_playwright_endpoint(endpoint) do
            true ->
              ...
              # We can use this port since it's already our server
              true

            false ->
              Logger.info("Port #{port} is occupied by non-Playwright service, trying next port")
              # Port is occupied by something else, try next port
              false
          end
      end
    end) || List.last(ports)
    ...
    try do
      ...
      case System.cmd("powershell.exe", [
        ...
        {chrome_path, 0} ->
          ...
          case System.cmd("powershell.exe", [
                 "-Command",
                 "Get-ChildItem '#{chrome_path}' -Directory | Where-Object {$_.Name -match '^(Default|Profile )' -or $_.Name -eq 'Profile 1'} | Select-Object Name,FullName | ForEach-Object {\"$($_.Name)|$($_.FullName)\"}"
               ]) do
            {output, 0} ->
              profiles =
                output
